      ...
      def prep_plates_for_export(group)
        begin
          ...
          plates.each do |group|
            ...
            labels = @plate_g_cpy.entities.select{|grp| (grp.is_a? Sketchup::Group) && (grp.name == group.definition.name)}
            ...
            label_vector = Geom::Vector3d.new [0.25,0.25,0]

            @plate_g_cpy.entities.erase_entities(vert_edges.to_a)
            label = @plate_g_cpy.entities.add_text("#{Q_LABEL}=#{dictionary[Q_LABEL]}\n#{PN_LABEL}=#{dictionary[PN_LABEL]}\n#{M_LABEL}=#{dictionary[M_LABEL]}\n#{TH_LABEL}=#{dictionary[TH_LABEL]}", place, label_vector)

            ip = Sketchup::InstancePath.new([labels[0], labels[0].entities[0]])

            label.attached_to = [ip, ip[1].end.position]
